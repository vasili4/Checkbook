...
print `date`;
open(TABLES, "psql -p 5432 -A -t -c \"select viewname from pg_views where schemaname = 's' and viewname = 'aggregateonasst_cat_typeandcfda_program_num' order by 1\" |") || die "Can't Get List of Tables";
