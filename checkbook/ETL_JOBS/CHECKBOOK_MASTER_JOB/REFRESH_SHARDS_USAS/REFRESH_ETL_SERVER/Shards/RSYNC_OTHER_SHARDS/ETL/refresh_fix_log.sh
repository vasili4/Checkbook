...
continue=`psql -A -t -h mdw -U gpadmin --dbname USASpending -c "select fix_flag from refresh_shards_status where latest_flag = 1 and shard_name = '$host'"`
...
then
	...
	then
		...
		then
			...
			psql -h mdw -U gpadmin --dbname USASpending -c "update refresh_shards_status set fix_flag = 1, fix_end_date = now(), refresh_end_date = now() where latest_flag = 1 and shard_name = '$host'"
		fi
	else
		echo "We have errors - not updating the table to move forward"
	fi
