...
continue=`psql -A -t -h mdw -U gpadmin --dbname USASpending -c "select sql_flag from refresh_shards_status where latest_flag = 1 and shard_name = '$host'"`
...
then
	...
	psql -h mdw -U gpadmin --dbname USASpending -c "update refresh_shards_status set fix_flag = 0, fix_start_date = now() where latest_flag = 1 and shard_name = '$host'"
