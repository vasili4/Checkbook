...
my $SQL = "
	set maintenance_work_mem = 131072;
	begin;
...
print `date`;
open(TABLES, "psql -p 5432 -A -t -c \"select viewname from pg_views where schemaname = 's' order by 1 desc\" |") || die "Can't Get List of Tables";
