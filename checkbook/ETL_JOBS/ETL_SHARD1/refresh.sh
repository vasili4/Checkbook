...
psql -h mdw1 -U gpadmin --dbname mmnyccheckbook_athu -c "update refresh_shards_status set sql_flag = 0, sql_start_date = now() where latest_flag = 1 and shard_name = '$host'"
...
then
        ...
        psql -h mdw1 -U gpadmin --dbname mmnyccheckbook_athu -c "update refresh_shards_status set sql_flag = 1, sql_end_date = now(), refresh_end_date = now() where latest_flag = 1 and shard_name = '$host'"
fi
