/* Functions defined
	...
	associateMAGToDO1
	updateForeignKeysForDO1Vendors
...
BEGIN
	...
	CREATE TEMPORARY TABLE tmp_fk_values_do1 (uniq_id bigint, document_code_id smallint,agency_history_id smallint,
					      ...
					      source_created_date_id smallint,source_updated_date_id smallint,original_term_begin_date_id smallint,
	...
	INSERT INTO tmp_fk_values_do1(uniq_id,document_function_code_id)
	SELECT	a.uniq_id, b.document_function_code_id
	FROM etl.stg_con_do1_header a JOIN ref_document_function_code b ON a.doc_func_cd = b.document_function_code_id;
	...
	INSERT INTO tmp_fk_values_do1(uniq_id,procurement_type_id)
	SELECT	a.uniq_id, b.procurement_type_id
	FROM etl.stg_con_do1_header a JOIN ref_procurement_type b ON a.prcu_typ_id = b.procurement_type_id;
	...
	INSERT INTO tmp_fk_values_do1(uniq_id,source_updated_date_id)
	SELECT	a.uniq_id, b.date_id
	...
	FROM	(SELECT uniq_id, max(document_code_id) as document_code_id ,
				 ...
				 max(document_function_code_id) as document_function_code_id, 
				 max(record_date_id) as record_date_id,
				 max(procurement_type_id) as procurement_type_id,
...
CREATE OR REPLACE FUNCTION etl.associateMAGToDO1() RETURNS INT AS $$
DECLARE
...
BEGIN
	...
	FROM	
		...
		FROM tmp_do1_mag a JOIN history_all_master_agreement b ON a.mag_document_id = b.document_id 
	...
	*/
	...
	FROM   tmp_do1_mag a JOIN tmp_new_do1_mag_con b ON a.uniq_id = b.uniq_id
		JOIN tmp_do1_mag c ON c.mag_document_code = a.mag_document_code
...
BEGIN
	...
	CREATE TEMPORARY TABLE tmp_do1_vendor_new(uniq_id bigint, vendor_customer_code varchar)
	DISTRIBUTED BY (uniq_id);
	...
	UPDATE etl.stg_con_do1_vendor a
	SET	vendor_history_id = b.vendor_history_id
	FROM	tmp_fk_values_do1_vendor b 
	WHERE	a.uniq_id = b.uniq_id;
	
	RETURN 1;
EXCEPTION
	WHEN OTHERS THEN
	RAISE NOTICE 'Exception Occurred in updateForeignKeysForDO1Vendors';
	RAISE NOTICE 'SQL ERRROR % and Desc is %' ,SQLSTATE,SQLERRM;	
...
CREATE OR REPLACE FUNCTION etl.updateForeignKeysForDOInAccLine() RETURNS INT AS $$
DECLARE
BEGIN
	-- UPDATING FK VALUES IN ETL.STG_CON_DO1_ACCOUNTING_LINE
	
	CREATE TEMPORARY TABLE tmp_fk_values_do1_acc_line(uniq_id bigint,event_type_id smallint, fund_class_id smallint,agency_history_id smallint,
							department_history_id int, expenditure_object_history_id integer,budget_code_id integer)
	DISTRIBUTED BY (uniq_id);
	
	-- FK:event_type_id

	INSERT INTO tmp_fk_values_do1_acc_line(uniq_id,event_type_id)
	SELECT	a.uniq_id, b.event_type_id
	FROM etl.stg_con_do1_accounting_line a JOIN ref_event_type b ON a.evnt_typ_id = b.event_type_code;	
	
	-- FK:fund_class_id

	INSERT INTO tmp_fk_values_do1_acc_line(uniq_id,fund_class_id)
	SELECT	a.uniq_id, b.fund_class_id
	FROM etl.stg_con_do1_accounting_line a JOIN ref_fund_class b ON a.fund_cd = b.fund_class_code;	

	-- FK:agency_history_id

	INSERT INTO tmp_fk_values_do1_acc_line(uniq_id,agency_history_id)
	SELECT	a.uniq_id, max(c.agency_history_id) 
	FROM etl.stg_con_do1_accounting_line a JOIN ref_agency b ON a.dept_cd = b.agency_code
		JOIN ref_agency_history c ON b.agency_id = c.agency_id
	GROUP BY 1	;	
	...
	FROM etl.stg_con_do1_accounting_line a JOIN ref_department b ON a.appr_cd = b.department_code AND a.bfy = b.fiscal_year
	...
	FROM etl.stg_con_do1_accounting_line a JOIN ref_expenditure_object b ON a.obj_cd = b.expenditure_object_code AND a.bfy = b.fiscal_year
	...
	FROM etl.stg_con_do1_accounting_line a JOIN ref_budget_code b ON a.func_cd = b.budget_code AND a.bfy=b.fiscal_year
		JOIN ref_agency d ON a.dept_cd = d.agency_code AND b.agency_id = d.agency_id
		JOIN ref_fund_class e ON a.fund_cd = e.fund_class_code AND e.fund_class_id = b.fund_class_id;	
...
BEGIN
	...
	IF l_fk_update = 1 THEN
		l_fk_update := etl.associateMAGToDO1();
	ELSE
		RETURN -1;
	END IF;
	...
	IF l_fk_update = 1 THEN	 
		l_fk_update := etl.updateForeignKeysForDOInAccLine();	
	ELSE
		RETURN -1;
	END IF;
	...
	CREATE TEMPORARY TABLE tmp_fk_values_do1_commodity(uniq_id bigint,commodity_type_id int)
	DISTRIBUTED BY (uniq_id);	
	
	-- FK:commodity_type_id

	INSERT INTO tmp_fk_values_do1_commodity(uniq_id,commodity_type_id)
	SELECT	a.uniq_id, b.commodity_type_id
	FROM etl.stg_con_do1_commodity a JOIN ref_commodity_type b ON a.ln_typ = b.commodity_type_id;
	
	UPDATE etl.stg_con_do1_commodity a
	SET	commodity_type_id = b.commodity_type_id
	FROM	tmp_fk_values_do1_commodity b
	WHERE 	a.uniq_id = b.uniq_id;
	...
	*/
	...
	INSERT INTO tmp_do1_con(uniq_id,agency_history_id,doc_id,doc_vers_no,privacy_flag)
	...
	CREATE TEMPORARY TABLE tmp_old_do1_con(uniq_id bigint,agreement_id bigint, action_flag char(1), latest_flag char(1),privacy_flag char(1),old_agreement_ids varchar)
	DISTRIBUTED BY (uniq_id);
	...
	FROM	
		...
		FROM etl.stg_con_do1_header a JOIN history_all_agreement b ON a.doc_id = b.document_id AND a.document_code_id = b.document_code_id
		...
		GROUP BY 1) inner_tbl JOIN etl.stg_con_do1_header d ON inner_tbl.uniq_id = d.uniq_id;
	...
	CREATE TEMPORARY TABLE tmp_new_do1_con(uniq_id bigint,agreement_id bigint, action_flag char(1), latest_flag char(1))
	DISTRIBUTED BY (uniq_id);
	...
	FROM	tmp_do1_con a JOIN etl.stg_con_do1_header b ON a.doc_id = b.doc_id AND a.agency_history_id = b.agency_history_id
	...
	INSERT INTO etl.agreement_id_seq(uniq_id)
	SELECT uniq_id
	FROM	tmp_new_do1_con;
	
	UPDATE tmp_new_do1_con a
	SET	agreement_id = b.agreement_id
	...
	CREATE TEMPORARY TABLE tmp_do1_deletion(agreement_id bigint)
	DISTRIBUTED BY (agreement_id);
	
	INSERT INTO tmp_do1_deletion
	SELECT  unnest(string_to_array(old_agreement_ids,','))::int
	FROM	tmp_do1_con
	WHERE	action_flag = 'I'
		AND latest_flag ='Y';	
		
	DELETE FROM agreement_accounting_line WHERE agreement_id IN (select agreement_id from tmp_do1_deletion);
	DELETE FROM all_agreement_accounting_line WHERE agreement_id IN (select agreement_id from tmp_do1_deletion);
	
	DELETE FROM agreement_commodity WHERE agreement_id IN (select agreement_id from tmp_do1_deletion);
	DELETE FROM all_agreement_commodity WHERE agreement_id IN (select agreement_id from tmp_do1_deletion);
	
	DELETE FROM agreement_worksite WHERE agreement_id IN (select agreement_id from tmp_do1_deletion) AND master_agreement_yn='N';
	...
	*/
	...
	DELETE FROM history_all_agreement_commodity WHERE agreement_id IN (SELECT agreement_id FROM tmp_do1_deletion);
	...
	SELECT a.*
	FROM   all_agreement_commodity a JOIN tmp_do1_con b ON a.agreement_id = b.agreement_id
	WHERE	privacy_flag = 'F';
	...
	SELECT a.*
	FROM   history_all_agreement_commodity a JOIN tmp_do1_con b ON a.agreement_id = b.agreement_id
	WHERE	privacy_flag = 'F';	
