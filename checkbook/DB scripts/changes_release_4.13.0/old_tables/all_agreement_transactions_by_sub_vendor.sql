...
DROP TABLE IF EXISTS all_agreement_transactions_by_sub_vendor;

CREATE TABLE all_agreement_transactions_by_sub_vendor (
	agreement_id bigint,
...
TRUNCATE TABLE all_agreement_transactions_by_sub_vendor;

-- SELECT * FROM all_agreement_transactions_by_prime_vendor
INSERT INTO all_agreement_transactions_by_sub_vendor (
	agreement_id,
...
a.agency_id,
a.agency_name,
b.vendor_id as prime_vendor_id,
b.vendor_name as prime_vendor_name,
b.minority_type_id as prime_minority_type_id,
CASE
...
CASE WHEN d.document_code in ('CT1', 'CT2', 'CTA1') THEN b.scntrc_status ELSE 5 END AS scntrc_status,
CASE WHEN d.document_code in ('CT1', 'CT2', 'CTA1') THEN b.scntrc_status_name ELSE 'N/A' END AS scntrc_status_name,
a.award_method_id,
a.award_method_code,
a.award_method_name,
a.award_size_id,
NULL AS expenditure_object_codes,
'N/A' AS expenditure_object_names,
a.industry_type_id,
...
CASE
	WHEN b.minority_type_id IN (7,11) THEN 'P:'||CAST(vendor.vendor_customer_code as text)
	ELSE 'PM:'||CAST(vendor.vendor_customer_code as text)
END ||
CASE
	WHEN a.minority_type_id IS NULL THEN ''
	WHEN a.minority_type_id IN (7,11) THEN ',S:'||CAST(subvendor.vendor_customer_code as text)
	ELSE ',SM:'||CAST(subvendor.vendor_customer_code as text)
END AS prime_sub_vendor_code_by_type,

CASE
	WHEN b.minority_type_id IN (7,11) THEN 'P:'||CAST(b.vendor_name as text)
	ELSE 'PM:'||CAST(b.vendor_name as text)
END ||
CASE
	WHEN a.minority_type_id IS NULL THEN ''
	WHEN a.minority_type_id IN (7,11) THEN ',S:'||CAST(a.vendor_name as text)
	ELSE ',SM:'||CAST(a.vendor_name as text)
END AS prime_sub_vendor_name_by_type,

CASE
	WHEN b.minority_type_id IN (7,11) THEN 'P:'||b.minority_type_id
	ELSE 'PM:'||b.minority_type_id
END ||
CASE
	WHEN a.minority_type_id IS NULL THEN ''
	WHEN a.minority_type_id IN (7,11) THEN ',S:'||a.minority_type_id
	ELSE ',SM:'||a.minority_type_id
END AS prime_sub_minority_type_id,
b.vendor_type||':'||b.minority_type_id||':'||CAST(vendor.vendor_customer_code as text)||':'||b.vendor_name ||
(CASE
                WHEN a.vendor_name IS NULL THEN ''
                ELSE ','||a.vendor_type||':'||a.minority_type_id||':'||CAST(subvendor.vendor_customer_code as text)||':'||a.vendor_name
...
(
	...
	a.minority_type_id,
	a.scntrc_status,
	a.scntrc_status_name
	FROM all_agreement_transactions a
	WHERE a.latest_flag = 'Y'
	AND a.is_prime_or_sub IN ('P')
...
) vendor on vendor.vendor_id = b.vendor_id
LEFT JOIN
(
...
) subvendor on subvendor.vendor_id = a.vendor_id
