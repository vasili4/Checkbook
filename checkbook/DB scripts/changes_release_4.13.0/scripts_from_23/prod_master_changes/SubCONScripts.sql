...
BEGIN
	...
	*/
    ...
    SELECT  original_agreement_id, document_version, document_code_id, agency_history_id, 
            ...
            (CASE WHEN scntrc_status = 2  THEN (CASE WHEN a.contract_number NOT IN (select distinct contract_number from subcontract_details) THEN 6 ELSE 0 END) ELSE 0 END) AS aprv_sta,
            (CASE WHEN scntrc_status = 2 THEN (CASE WHEN a.contract_number NOT IN (select distinct contract_number from subcontract_details) THEN 'No Subcontract Information Submitted' ELSE 'N/A' END) ELSE 'N/A' END ) AS aprv_sta_name 
    ...
    SELECT  original_agreement_id, document_version, document_code_id, agency_history_id, 
     ...
     FROM sub_agreement_snapshot_cy a LEFT JOIN ref_date b ON a.last_modified_date::date = b.date
        ...
        LEFT JOIN (select original_agreement_id, contract_number from history_agreement where latest_flag = 'Y') e ON a.contract_number = e.contract_number;
			...
			LEFT JOIN
			(
				SELECT a.original_agreement_id, a.contract_number, a.vendor_name, a.vendor_id,a.aprv_sta as aprv_sta_id, a.minority_type_id
				FROM all_agreement_transactions a
